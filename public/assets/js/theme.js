...
var navbarInit = function navbarInit() {
    ...
    if (navbar) {
        ...
        var transition = 'background-color,padding 2.35s ease';
        navbar.style.backgroundImage = 'none'; // Change navbar background color on scroll

        window.addEventListener(Events.SCROLL, function () {
            var scrollTop = html.scrollTop;
            var alpha = scrollTop / windowHeight * 2.35; // Add class on scroll
...
/* -------------------------------------------------------------------------- */
var animateHeroTitle = function animateHeroTitle() {
    var textWrapper = document.querySelector('.hero-animated');
    textWrapper.innerHTML = textWrapper.textContent.replace(/\S/g, "<span class='letter'>$&</span>");

    anime.timeline({
        loop: true
    }).add({
        targets: '.hero-animated .letter',
        translateY: [100, 0],
        translateZ: 0,
        opacity: [0, 1],
        easing: "easeOutExpo",
